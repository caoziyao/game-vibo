...
import GuaScene  from '../../game/game_scene'
...
export default class SceneMain extends GuaScene {
    ...
    setupInput() {
        ...
        wx.onTouchStart(function (e) {
            ...
            // console.log('ssss', touches,  e.touches, e)
            var x = touches.clientX
            var y = touches.clientY
            // 检查是否点中了 ball
            // console.log('restaretaaa', x, y, self.end, self.hasPoint(x, y))
            if (self.hasPoint(x, y) && self.end) {
                // enableDrag = true
                self.end = false
                console.log('restaret')
                self.restartGame()
                window.paused = false
